...
def capture_array(cam):
    ...
    if not cam.camera.started:
        cam.camera.start()

    stream = io.BytesIO()
    cam.camera.capture_file(stream, format='jpeg')
    stream.seek(0)
...
def detect_objects(cam, model, device, imgsz, target_classes=None):
    ...
    pred = non_max_suppression(pred, conf_thres=0.6, iou_thres=0.45, classes=target_classes)
