...
class Car:
    ...
    def mode_sonic_clamp(self):
        """
        Drive forward until ultrasonic distance enters (5,12] cm,
        then perform the existing clamp-up + clamp-down sequence, and stop.
        """
        distance = self.sonic.get_distance()
        print(f"[SONIC_CLAMP] distance = {distance} cm")

        # 1) Too far: keep driving forward
        if distance == 0 or distance > 12:
            self.motor.setMotorModel(500, 500)

        # 2) In clamp zone: stop and perform clamp sequence
        elif 5 < distance <= 12:
            self.motor.setMotorModel(0, 0)
            print("[SONIC_CLAMP] entering clamp zone, starting clamp-up")
            self.set_mode_clamp(1)
            while self.get_mode_clamp() == 1:
                self.mode_clamp()
            print("[SONIC_CLAMP] clamp-up complete, starting clamp-down")
            self.set_mode_clamp(2)
            while self.get_mode_clamp() == 2:
                self.mode_clamp()
            self.motor.setMotorModel(0, 0)
            print("[SONIC_CLAMP] clamp sequence complete, stopped")

        # 3) Too close: back off slightly
        #else:
        #    self.motor.setMotorModel(-500, -500)
    ...
    def mode_clamp_up(self):
        ...
        if self.clamp_mode == 1:
            ...
            if distance <= 5:
                self.motor.setMotorModel(-300, -300)  # Move backward slowly
            elif distance > 5 and distance < 7.5:
                self.motor.setMotorModel(-200, -200)    # Move backward faster
                ...
                self.motor.setMotorModel(200, 200)      # Move forward slowly
            elif distance >= 11:
                self.motor.setMotorModel(300, 300)    # Move forward quickly
            # Sleep for a short duration
            time.sleep(0.05) 
...
def test_car_sonic_clamp():
    car = Car()
    try:
        while True:
            car.mode_sonic_clamp()
    except KeyboardInterrupt:
        car.close()
        print("\nEnd of auto-clamp test")
